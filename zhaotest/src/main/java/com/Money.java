 ...
 *
 * <p>
 * 货币类中封装了货币金额和币种。目前金额在内部是BigDecimal类型表示，
 * 单位是元。
 * 小数点精确到所属币种的最小货币单位（对人民币是分）。
 *
 * <p>
 * 目前，货币实现了以下主要功能：<br>
 * <ul>
...
public class Money implements Serializable, Comparable<Object> {
    ...
    /**
     * 缺省的取整模式，为<code>RoundingMode.HALF_UP</code>
     * （四舍五入）。
     */
     ...
     */
    public long getCent() {
        return amount.multiply(getCentFactor()).longValue();
    }
    ...
    public void setCent(long cent) {
        this.setAmount(new BigDecimal(cent).divide(getCentFactor()));
    }
    ...
    /**
     * 设定本货币对象代表的币种。不允许在代码中调用，仅用于venus框架反序列化对象时使用。
     * 
     * @deprecated
     */
     ...
     */
    public int hashCode() {
        long cent = amount.multiply(getCentFactor()).longValue();
    ...
    public Money add(Money other) {
        assertSameCurrencyAs(other);
        return newMoneyWithSameCurrency(amount.add(other.amount));
    }
    ...
    public Money subtract(Money other) {
        ...
        return newMoneyWithSameCurrency(amount.subtract(other.amount));
    }
    ...
    public Money multiply(long val) {
        return newMoneyWithSameCurrency(this.amount.multiply(new BigDecimal(val)));
    }
    ...
    public Money multiply(double val) {
        return newMoneyWithSameCurrency(amount.multiply(new BigDecimal(val)));
    }
    ...
    public Money multiply(BigDecimal val, RoundingMode roundingMode) {
        BigDecimal newCent = amount.multiply(val);

        return newMoneyWithSameCurrency(newCent, roundingMode);
    }
    ...
    public Money multiplyBy(BigDecimal val, RoundingMode roundingMode) {
        this.setAmount(amount.multiply(val), roundingMode);
    ...
    public Money divide(double val) {
        return newMoneyWithSameCurrency(amount.divide(new BigDecimal(val)));
    }
    ...
    public Money divideBy(double val) {
        this.amount = amount.divide(new BigDecimal(val));
    ...
    public Money divide(BigDecimal val, RoundingMode roundingMode) {
        BigDecimal newAmount = amount.divide(val);

        return newMoneyWithSameCurrency(newAmount, roundingMode);
    }
    ...
    public Money divideBy(BigDecimal val, RoundingMode roundingMode) {
        this.setAmount(amount.divide(val), roundingMode);
    ...
    public Money[] allocate(int targets) {
        ...
        for (int i = 0; i < remainder; i++) {
            results[i] = newMoneyWithSameCurrency(new BigDecimal(highResult)
                .divide(getCentFactor()));
        }

        for (int i = remainder; i < targets; i++) {
            results[i] = newMoneyWithSameCurrency(new BigDecimal(lowResult).divide(getCentFactor()));
        }
    ...
    public Money[] allocate(long[] ratios) {
        ...
        for (int i = 0; i < results.length; i++) {
            results[i] = (getCent() * ratios[i]) / total;
            remainder -= results[i];
        }
        ...
        for (int i = 0; i < ratios.length; i++) {
            ret[i] = newMoneyWithSameCurrency(new BigDecimal(results[i]).divide(getCentFactor()));
        }
    ...
    public String formatWithCode() {
        ...
        sb.append(currency.getCurrencyCode()).append(nf.format(amount));
    ...
    protected Money newMoneyWithSameCurrency(BigDecimal amount) {
        return newMoneyWithSameCurrency(amount, DEFAULT_ROUNDING_MODE);
    }
    ...
    public String dump() {
        ...
        sb.append("amount = ").append(amount).append(lineSeparator);
