                ...
                if (flag == rule.name) {
                    ...
                    if (rule.validate.call(field, value) == x) {
                        ...
                        if (el.attr('type')!=null && el.attr('type').toLowerCase()=='file'){
                            errorMsg = (msg == null)?'请选择Excel或TXT文件':msg;
                        }
                        else{
					...
					if (el.attr('type')!=null && el.attr('type').toLowerCase()=='file'){
						var k = el.val().substr(el.val().indexOf(".")+1);
						if(k.toLowerCase()!="xls"&&k.toLowerCase()!="xlsx"&&k.toLowerCase()!='txt'){
							error=true;
							errorMsg="请选择Excel或TXT文件";
						}
					}
                }
            }
